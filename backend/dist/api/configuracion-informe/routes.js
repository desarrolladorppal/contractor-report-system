...
router.get('/', async (req, res) => {
    try {
        ...
        const config = await model_1.ConfiguracionInforme.findOne({
...
router.post('/', async (req, res) => {
    try {
        const { usuarioId, contratoId, columnas, campos, actividades, bloquesTexto, firmas, lugarFecha, dependenciaContratante, seguridadSocial } = req.body;
        if (!usuarioId || !contratoId) {
            return res.status(400).json({ error: 'usuarioId y contratoId son requeridos' });
        }
        // Verificar si ya existe una configuración para este contrato
        const existingConfig = await model_1.ConfiguracionInforme.findOne({
            usuarioId: usuarioId.toString(),
            contratoId: contratoId.toString()
        });
        if (existingConfig) {
        ...
        const nuevaConfig = new model_1.ConfiguracionInforme({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (actividades)
            config.actividades = actividades;
        if (bloquesTexto)
            config.bloquesTexto = bloquesTexto;
        if (firmas)
            config.firmas = firmas;
        if (lugarFecha !== undefined)
            config.lugarFecha = lugarFecha;
        if (dependenciaContratante !== undefined)
            config.dependenciaContratante = dependenciaContratante;
        if (seguridadSocial)
            config.seguridadSocial = seguridadSocial;
        if (contratoId)
            config.contratoId = contratoId;
        await config.save();
        res.json(config);
    }
    catch (error) {
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const config = await model_1.ConfiguracionInforme.findOneAndDelete({
            id: id,
            usuarioId: usuarioId.toString()
        });
        if (!config) {
