...
router.get('/', async (req, res) => {
    try {
        ...
        const contratos = await model_1.Contract.find({ usuarioId }).sort({ creadoEn: -1 });
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const contrato = await model_1.Contract.findById(req.params.id);
        if (!contrato) {
...
router.post('/', async (req, res) => {
    try {
        ...
        const nuevoContrato = new model_1.Contract({
            ...req.body,
            creadoEn: new Date(),
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const contrato = await model_1.Contract.findByIdAndUpdate(req.params.id, { ...req.body, actualizadoEn: new Date() }, { new: true });
        if (!contrato) {
            console.log('❌ Contrato no encontrado');
            return res.status(404).json({ error: 'Contrato no encontrado' });
        }
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const contrato = await model_1.Contract.findByIdAndDelete(req.params.id);
        if (!contrato) {
            console.log('❌ Contrato no encontrado');
            return res.status(404).json({ error: 'Contrato no encontrado' });
        }
