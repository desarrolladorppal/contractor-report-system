...
router.get('/', async (req, res) => {
    try {
        ...
        let configuracion = await model_1.Configuracion.findOne({
        ...
        if (!configuracion) {
            ...
            const nuevaConfig = new model_1.Configuracion({
                usuarioId: usuarioId.toString(),
                contratoId: contratoId.toString(),
                reportes: {
                    frecuencia: 'mensual',
...
router.put('/:contratoId', async (req, res) => {
    try {
        ...
        const configuracion = await model_1.Configuracion.findOneAndUpdate({ contratoId: contratoId.toString(), usuarioId: usuarioId.toString() }, { ...req.body }, { new: true, upsert: true });
...
router.post('/:contratoId/reset', async (req, res) => {
    try {
        ...
        const configuracion = await model_1.Configuracion.findOneAndUpdate({ contratoId: contratoId.toString(), usuarioId: usuarioId.toString() }, configuracionDefault, { new: true, upsert: true });
